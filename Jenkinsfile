pipeline {
  ...
  stages {
    stage ('Build Docker Image') {
      ...
      steps {
        script {
          app = docker.build('richardluo0506/devops:latest')
        }
      }
    }
      ...
      steps {
        ...
        sshagent (credentials: ['5e322410-00af-4ee3-b2ff-6bf5ffd0f194']) {
          script {
            ...
            sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker pull richardluo0506/devops:latest:latest\""
            try {
